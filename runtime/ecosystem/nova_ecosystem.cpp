...
// ---- Nova runtime ABI this file depends on ----
// Previously this block hand-declared its own copy of the runtime ABI
// instead of including nova_rt.hpp, and that copy had drifted out of sync
// with the canonical header (int vs bool for nova_rt_to_bool /
// nova_rt_from_bool). Two translation units declaring the same extern "C"
// symbol with different signatures is undefined behavior — a real,
// intermittent source of the segfaults this file used to need a pre-build
// patch script to work around. Including the canonical header directly
// makes that drift structurally impossible: there is now exactly one
// declaration of every nova_rt_* symbol in the whole build.
#include "nova_rt.hpp"
